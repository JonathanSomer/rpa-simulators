...
class HPASimple(ODE):
    """Simplified HPA axis model without gland dynamics.

    A 3-state version of the HPA model that assumes constant gland masses
    (P=1, A=1). Useful for faster simulations when gland dynamics are not
    of interest.
    ...
    Equations:
        dx1/dt = γ_x1 * (I1*u(t)*MR(C3*x3)*GR(C3*x3) - A1*x1)
        dx2/dt = γ_x2 * (I2*(C1*x1)*GR(C3*x3) - A2*x2)
        dx3/dt = γ_x3 * (I3*(C2*x2) - A3*x3)

    Control vector order: [I1, I2, I3, C1, C2, C3, A1, A2, A3]
    """

    name = "HPA Axis Simple (No Glands)"
    variable_names = ["CRH", "ACTH", "Cortisol"]
    fixed_param_names = ["gamma_x1", "gamma_x2", "gamma_x3", "KGR", "nGR"]
    ...
    def __init__(
        ...
        """Initialize simplified HPA ODE with parameters.

        Args:
            fixed_params: [gamma_x1, gamma_x2, gamma_x3, KGR, nGR]
                         Uses physiological defaults if not provided.
            stressor: Function u(t) representing external stressor input.
                     Defaults to constant 1.0.
        """
        if fixed_params is None:
            fixed_params = torch.tensor([
                np.log(2) / 4 * 24 * 60,    # gamma_x1
                np.log(2) / 20 * 24 * 60,   # gamma_x2
                np.log(2) / 80 * 24 * 60,   # gamma_x3
                4.0,                         # KGR
                3.0,                         # nGR
            ])
    ...
    def forward(
        ...
        """Compute dx/dt for the simplified HPA system.

        Args:
            t: Time tensor
            x: State tensor [CRH, ACTH, Cortisol]
            differentiable_params: Not used (all params are fixed)
            fixed_params: [gamma_x1, gamma_x2, gamma_x3, KGR, nGR]
            control: Control tensor [I1, I2, I3, C1, C2, C3, A1, A2, A3]
                    Defaults to [1, 1, 1, 1, 1, 1, 1, 1, 1] if not provided.
        ...
        Returns:
            dx/dt tensor [dCRH/dt, dACTH/dt, dCortisol/dt]
        """
        assert fixed_params is not None, "Fixed params required"
        assert len(fixed_params) == 5, "Expected 5 fixed params"
        assert len(x) == 3, "Expected state [CRH, ACTH, Cortisol]"

        # Unpack state
        x1, x2, x3 = x[0], x[1], x[2]

        # Unpack fixed parameters
        gamma_x1, gamma_x2, gamma_x3, KGR, nGR = fixed_params
        ...
        # Effective cortisol seen by receptors
        x3_eff = C3 * x3

        # Compute derivatives (P=1, A=1 assumed)
        dx1_dt = gamma_x1 * (I1 * u * MR(x3_eff) * GR(x3_eff) - A1 * x1)
        dx2_dt = gamma_x2 * (I2 * (C1 * x1) * GR(x3_eff) - A2 * x2)
        dx3_dt = gamma_x3 * (I3 * (C2 * x2) - A3 * x3)

        return torch.stack([dx1_dt, dx2_dt, dx3_dt])

    def __str__(self) -> str:
        """Return string representation with equations and parameters."""
        gamma_x1, gamma_x2, gamma_x3, KGR, nGR = self.fixed_params

        return (
            f"{self.name}\n\n"
            f"State Variables:\n"
            f"  x1: CRH, x2: ACTH, x3: Cortisol\n\n"
            f"Equations (P=1, A=1):\n"
            f"  dx1/dt = γ_x1 * (I1*u(t)*MR(C3*x3)*GR(C3*x3) - A1*x1)\n"
            f"  dx2/dt = γ_x2 * (I2*(C1*x1)*GR(C3*x3) - A2*x2)\n"
            f"  dx3/dt = γ_x3 * (I3*(C2*x2) - A3*x3)\n\n"
            f"Parameters:\n"
            f"  γ_x1 = {gamma_x1:.2f}, γ_x2 = {gamma_x2:.2f}, γ_x3 = {gamma_x3:.2f}\n"
            f"  K_GR = {KGR:.1f}, n_GR = {nGR:.1f}\n\n"
            f"External Input:\n"
