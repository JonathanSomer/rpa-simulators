...
class NFL(ODE):
    """Negative Feedback Loop (NFL) system with control-modulated parameters.

    A generic 3-node feedback network with activation and degradation reactions.
    All parameters can be modulated by control inputs (like ABControlled).
    ...
    Equations:
        ...
        dB/dt = C * k_CB * (1-B)/((1-B)+K_CB) - F_B * k'_F_BB * B/(B+K'_F_BB)
    ...
    Fixed Parameters (14 total):
        ...
        k_CB, K_CB: Activation of B by C (rate and affinity)
    ...
    Control vector order: [u_k_IA, u_K_IA, u_k'_F_AA, u_K'_F_AA, u_k_CB, u_K_CB,
                          u_k'_F_BB, u_K'_F_BB, u_k_AC, u_K_AC, u_k'_BC, u_K'_BC,
                          u_F_A, u_F_B]
    """

    name = "Negative Feedback Loop (Controlled)"
    ...
    def __init__(
        ...
        """Initialize NFL ODE with parameters.

        Args:
            fixed_params: [k_IA, K_IA, k'_F_AA, K'_F_AA, k_CB, K_CB,
        ...
        if fixed_params is None:
            # Default parameters satisfying (1-B) >> K_CB and B >> K'_F_BB
            fixed_params = torch.tensor([
                ...
                1.0,    # k_CB: activation rate of B by C
                0.0001,   # K_CB: very small (so (1-B) >> K_CB)
                1.0,    # k'_F_BB: degradation rate of B by F_B
                0.0001,   # K'_F_BB: very small (so B >> K'_F_BB)
                ...
                1.0,    # F_B: basal enzyme for B
            ])

            # steady-state of C under saturation conditions is F_B * k'_F_BB / k_CB = 1.0 * 1.0 / 1.0 = 1.0
    ...
    def forward(
        ...
        """Compute dx/dt for the NFL system.
        ...
        # dB/dt = C * k_CB * (1-B)/((1-B)+K_CB) - F_B * k'_F_BB * B/(B+K'_F_BB)
        activation_B = C * k_CB * (1.0 - B) / ((1.0 - B) + K_CB)
    ...
    def __str__(self) -> str:
        ...
        return (
            ...
            f"  dB/dt = C*k_CB*u[4]*(1-B)/((1-B)+K_CB*u[5]) - F_B*u[13]*k'_F_BB*u[6]*B/(B+K'_F_BB*u[7])\n"
            ...
            f"  k_CB={params[4]:.2f}, K_CB={params[5]:.4f}\n"
            f"  k'_F_BB={params[6]:.2f}, K'_F_BB={params[7]:.4f}\n"
